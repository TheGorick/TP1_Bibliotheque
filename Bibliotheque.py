...
class Bibliotheque:
    """Classe de gestion des abonnées et des documents"""
    def __init__(self):
...
class Emprunts:
    """Classe pour gérer les emprunts et retour de livres."""
    ...
    def retourner_document(self, titre):
        #Permet de retourner un document emprunté et de le retirer de la liste des emprunts.
        for doc in self.bibliotheque.documents:
            if isinstance(doc, Livre) and doc.titre == titre:
                doc.retourner()
                break
        # Supprime l'emprunt correspondant dans la liste des emprunts
        self.emprunts = [emprunt for emprunt in self.emprunts if emprunt['titre'] != titre]
